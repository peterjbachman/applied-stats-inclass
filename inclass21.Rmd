...
## REMINDER 1: `k` denotes the current model (column `k` of `K`).

## REMINDER 2: `t` denotes the current prior presidential election (row `t` of `T`).

## REMINDER 3: `w_hat` is a vector of `K` elements.


w_hat2 <- (w_hat * dnorm(y[t], df[t,k], 1)/(sum(w_hat * dnorm(y[t], df[t,k], 1)))
```
