...
# NOTE: algorithm does not iterate past the first iteration. Can't find out why??
...
steps23 <- function(df = df, w_hat = wHat_it[i-1,]){
  ...
  w_hat2 <- sapply(
    X = 1:K,
    FUN = function(k){
      sum(z_hats[,k])/K # FIXED HERE! PREVIOUSLY `Ti`
    }
  )
  return(w_hat2)
}
...
# PRIOR ATTEMPT, DIDN'T WORK:
#sapply(
#  X = 2:20,
#  FUN = function(i){
#    steps23(df, wHat_it, i = i)
#  }
#)
