...
#creating a dataset that is a candidate-state dyad verison of our original primaryPolls dataset
primaryPolls_CandidateState <- primaryPolls %>% distinct(candidate_name, state, .keep_all = TRUE)

#getting the 5 highest polling candidates for each state
primaryPolls_CandidateState <- primaryPolls_CandidateState %>% arrange(desc(pct)) %>% group_by(state) %>% slice(1:5)

#comparing the size of our original primaryPolls dataset with our new candidate-dyad, 5 highest candidates dataset. 
...
```{r}
# Excercise from in-class lecture
